function cor = dic_T3(f,g,init,coor,conn,varargin)
% cor = dic_T3(f,g,init,coor,conn), correlate the image g on f using a T3
% mesh as defined by coor and conn, starting with init as initial guess.
% init : [ux(:), uy(:)]
% coor : [x(:), y(:)]
% conn : [n1(:), n2(:), n3(:)]
%
% cor = dic_T3(f,g,init,coor,conn,opt), where opt is a structure with
% optional fields:
...
if opt.plotflag
    if isempty(opt.Hax) || ~ishandle(opt.Hax)
        ...
        ha = axes('Position',[0.05,0.05,0.9,0.9],'DataAspectRatio',[1 1 1],'XColor','none','YColor','none','Xlim',[1, m],'Ylim',[1, n],'NextPlot','Add','YDir','reverse','Clipping','off');
        if isoctave
          hi = imagesc(f-g);
        else
          hi = imagesc(f-g,'AlphaData',mask);
        end
        hp = patch('Vertices',coor,'Faces',conn(:,Iface),'EdgeColor','k','FaceColor','none','EdgeAlpha',0.2);
        hp = patch('Vertices',coor,'Faces',conn(:,Iface),'EdgeColor','m','FaceColor','none','EdgeAlpha',0.2);
...
for bk = 1:Nb
    ...
    for it = 1:opt.maxit
        ...
        gt = interp2(X, Y, gb, X+Ux, Y+Uy, method, nan);
...
Ux = reshape(phi*a(Ix),n,m);
Uy = reshape(phi*a(Iy),n,m);
gt = interp2(X, Y, g, X+Ux, Y+Uy, method, nan);
res = f - gt;
res(~mask) = NaN;
r = rms(res(mask));
